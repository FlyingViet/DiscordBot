...
module.exports = {
    ...
    execute(message, args, con) {
        ...
        fetch('https://br1.api.riotgames.com/lol/summoner/v4/summoners/by-name/' + summonerNam.trim() + `?api_key=${key}`, {
            method: 'get',
        }).then(function(response){
            return response.json();
        }).then(function(data){
            var summonerId = data.id;
            return fetch('https://br1.api.riotgames.com/lol/spectator/v4/active-games/by-summoner/' + summonerId + `?api_key=${key}`, {method: 'get'}).then(function(response){
                return response.json();
                ...
                eb.setURL('https://br.op.gg/summoner/userName=' + summonerNam);
                if(r.status){
                    ...
                    const loop = async () =>{
                        ...
                        async.each(r.participants, async function(player){    
                            ...
                            fetch('https://br1.api.riotgames.com/lol/league/v4/entries/by-summoner/' + player.summonerId + `?api_key=${key}`, {method: 'get'}).then(function(response){
                                return response.json();
                            }).then(function(n){
                                count++;
